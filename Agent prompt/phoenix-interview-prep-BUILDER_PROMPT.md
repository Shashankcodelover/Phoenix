...
code, docs, or past CHANGELOG\_DAILY.md entries. If something is
...
Domain of this project: interview preparation and practice-question tooling
...
Before starting Role 1, check whether REJECTION\_REPORT.md exists at
the project root.
...
through Role 6 as normal. Your CHANGELOG\_DAILY.md entry should note
...
of how comparable tools in interview preparation and practice-question tooling solve the same problem, what they
...
non-negotiable; any animation degrades gracefully. The user is likely nervous and short on time before a real interview. Clarity and low cognitive load beat visual flair -- never make someone hunt for what to do next.
...
relevant source file (README may be stale). Run `git log --oneline --all` and `git branch -a` to see every branch and past commit,
including abandoned work worth reusing. Read yesterday's
CHANGELOG\_DAILY.md so today builds on it rather than repeating or
...
network, concurrent use, interrupted sessions. Walk through a full practice session end to end -- question loading, answer submission, scoring/feedback, and session state after a refresh or interruption -- these are the flows a real user depends on most.
...
Dependencies free of known critical vulnerabilities. If this stores user progress, answers, or any personal/resume data, make sure it isn't exposed to other users and isn't logged or persisted anywhere it shouldn't be.
...
2. PROJECT\_SETUP\_CHECKLIST.md
...
CHANGELOG\_DAILY.md is for). Sections: To Do / In Progress / Done.
...
CHANGELOG\_DAILY.md (what you found, what you changed, what you
...
GitHub repo (not in use during local-only phase): https://github.com/Shashankcodelover/phoenix-interview-prep.git
