...
export default function IdeaGenerationLab({ room, onProblemStatementLocked }) {
  ...
  return (
    <motion.div 
      ...
      className="w-full rounded-3xl bg-theme-card border border-emerald-500/30 p-5 sm:p-7 shadow-xl shadow-black/30 text-left space-y-6"
    >
      ...
      <div className="space-y-3 font-mono text-xs">
        ...
        <div>
          <input
            ...
            className="w-full px-4 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-950 border border-theme-glass text-theme-main focus:border-emerald-400 focus:outline-none placeholder:text-slate-400 transition-all"
          />
        </div>
        ...
        <motion.button
          ...
          disabled={loading}
          className="w-full py-3.5 rounded-xl font-mono font-bold text-xs bg-gradient-to-r from-emerald-500 via-teal-500 to-sky-500 hover:brightness-110 text-slate-950 transition-all shadow-lg shadow-emerald-500/25 flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
        >
          {loading ? (
            <motion.span 
      ...
      <AnimatePresence>
        {loading && (
          ...
          >
            <div className="p-5 rounded-2xl bg-slate-950 border-2 border-emerald-500/40 space-y-4 shadow-2xl mt-4">
                  ...
                  return (
                    <motion.div
                      ...
                      className={`p-3.5 rounded-xl border transition-all flex items-center justify-between ${
                        isCurrent
                          ? 'bg-emerald-950/60 border-emerald-400 text-emerald-200 font-bold shadow-md shadow-emerald-500/10'
                          : isCompleted
                    ...
                    >
                      ...
                      {isCurrent && <motion.span animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 2, ease: "linear" }} className="text-sky-400 font-bold inline-block">⟳</motion.span>}
                    </motion.div>
                  );
                })}
              </div>
      ...
      <AnimatePresence>
        {ideas.length > 0 && !loading && (
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="space-y-4 pt-4 border-t border-theme-glass"
          >
                    ...
                    >
                      ...
                      <div className="flex flex-wrap items-center justify-between gap-3 pt-3 font-mono text-xs border-t border-theme-glass">
                        <div className="flex items-center gap-2">
                          ...
                          >
                            <motion.span animate={hasUserVotedThis ? { scale: [1, 1.2, 1] } : {}}>🗳️</motion.span> 
                            {hasUserVotedThis ? '✓ Your Vote Cast' : 'Vote on Idea'} ({voteCount} / {memberCount} Votes)
                          </motion.button>
